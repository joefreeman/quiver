...
fn test_match_decimal_literal() {
    quiver().evaluate("[0.1, 0.2] %num.add =0.3").expect("Ok");
}
...
fn test_match_literal_no_match_is_nil() {
    quiver().evaluate("[0.1, 0.2] %num.add =0.4").expect("[]");
}
...
fn test_sqrt_of_surd_is_nil() {
    ...
    quiver().evaluate("2 %num.sqrt %num.sqrt").expect("[]");
}
...
fn test_surd_to_int_truncates_toward_zero() {
    ...
    quiver().evaluate("2 %num.sqrt %num.to_int").expect("1");
    quiver()
