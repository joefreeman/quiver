...
const DA: &str = "%dict.new [~, 0x0a59538016, 1] %dict.put [~, 0xfd3eb827ca, 2] %dict.put";
...
fn test_collision_count() {
    quiver().evaluate(&format!("{DA} %dict.count")).expect("2");
}
...
fn test_collision_overwrite() {
    ...
    quiver()
        .evaluate(&format!("{DA} [~, {KA}, 9] %dict.put [~, {KA}] %dict.get"))
        .expect("9");
    quiver()
        .evaluate(&format!("{DA} [~, {KA}, 9] %dict.put [~, {KB}] %dict.get"))
        .expect("2");
    quiver()
...
fn test_collision_remove_one() {
    ...
    quiver()
        .evaluate(&format!("{DA} [~, {KA}] %dict.remove [~, {KB}] %dict.get"))
        .expect("2");
    quiver()
        .evaluate(&format!("{DA} [~, {KA}] %dict.remove [~, {KA}] %dict.get"))
        .expect("[]");
    quiver()
...
fn test_collision_remove_absent() {
    ...
    quiver()
        .evaluate(&format!(r#"{DA} [~, "zz"] %dict.remove %dict.count"#))
        .expect("2");
}
...
// direct build. (Relies on structural equality via `=&`; see test_structural_equality_across_construction.)
...
fn test_put_get() {
    quiver()
        .evaluate(r#"%dict.new [~, "a", 1] %dict.put [~, "b", 2] %dict.put [~, "b"] %dict.get"#)
        .expect("2");
}
...
fn test_overwrite() {
    quiver()
        .evaluate(r#"%dict.new [~, "a", 1] %dict.put [~, "a", 99] %dict.put [~, "a"] %dict.get"#)
        .expect("99");
}
...
fn test_overwrite_keeps_count() {
    quiver()
        .evaluate(r#"%dict.new [~, "a", 1] %dict.put [~, "a", 9] %dict.put %dict.count"#)
        .expect("1");
}
...
fn test_remove_absent_noop() {
    quiver()
        .evaluate(r#"%dict.new [~, "a", 1] %dict.put [~, "z"] %dict.remove %dict.count"#)
        .expect("1");
}
...
fn test_remove_many_then_count() {
    quiver()
        .evaluate(&format!(r#"{FROM} [~, "echo"] %dict.remove %dict.count"#))
        .expect("11");
}
