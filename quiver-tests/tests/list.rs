...
#[test]
fn test_length() {
    quiver().evaluate("%list.new[] ~> %list.length").expect("0");

    quiver()
        .evaluate("%list.new[] ~> %list.prepend[~, 10] ~> %list.length")
        .expect("1");
...
fn test_concat() {
    quiver()
        .evaluate("[%list.new[], %list.new[]] ~> %list.concat")
        .expect("Nil");
